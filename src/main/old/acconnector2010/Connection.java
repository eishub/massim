...
/**
 * Represents a single connection to a MASSim-server. Each controllable entity
 * (cowboy) can have one connection.
 * 
...
public class Connection extends Socket implements Runnable {
	...
	/**
	 * Stores whether the connection is executing. Executing means listening for
	 * messages.
	 */
	private boolean executing = true;

	/**
	 * Contains the last action-id. It is sent if there is an action. Used for
	 * blocking action-methods.
	 */
	 ...
	 * 
	 * @param listener
	 *            is the object that is to be provided with incoming messages.
	 * @param host
	 *            is the hostname (URL or ip-address) of the MASSim-server.
	 * @param port
	 *            is the port of the MASSim-Server.
	 * @throws UnknownHostException
	 *             is thrown if the host is unknown.
	 * @throws IOException
	 *             is thrown if the connection cannot be established.
	 */
	public Connection(ConnectionListener listener, String host, int port)
	...
	public boolean authenticate(String username, String password) {
		...
		try {
			...
			System.err
					.println("unable to create new document for authentication.");
	 ...
	 * 
	 * @param doc
	 *            is the document to be sent.
	 * @throws IOException
	 *             is thrown if the document could not be sent.s
	 */
	private void sendDocument(Document doc) throws IOException {

		try {
			transformerfactory.newTransformer().transform(new DOMSource(doc),
	 ...
	 * 
	 * @param doc
	 *            is the document to be transformed.
	...
	private LinkedList<Percept> evaluateDocument(Document doc) {
		...
		if (type.equals("sim-start")) {
			...
			// corral-pos
			percept = new Percept("corralPos", new Numeral(new Integer(
					attributes.getNamedItem("corralx0").getNodeValue())),
					new Numeral(new Integer(attributes.getNamedItem("corraly0")
							.getNodeValue())),
					new Numeral(new Integer(attributes.getNamedItem("corralx1")
							.getNodeValue())),
					new Numeral(new Integer(attributes.getNamedItem("corraly1")
							.getNodeValue())));
			ret.add(percept);

			// grid-size
			percept = new Percept("gridSize", new Numeral(new Integer(
					attributes.getNamedItem("gsizex").getNodeValue())),
					new Numeral(new Integer(attributes.getNamedItem("gsizey")
							.getNodeValue())));
			ret.add(percept);

			// simlation id
			percept = new Percept("simId", new Identifier(attributes
					.getNamedItem("id").getNodeValue()));
			ret.add(percept);

			// line-of-sight
			percept = new Percept("lineOfSight", new Numeral(new Integer(
					attributes.getNamedItem("lineOfSight").getNodeValue())));
			ret.add(percept);

			// opponent
			percept = new Percept("opponent", new Identifier(attributes
					.getNamedItem("opponent").getNodeValue()));
			ret.add(percept);

			// steps
			percept = new Percept("steps", new Numeral(new Integer(attributes
					.getNamedItem("steps").getNodeValue())));
			ret.add(percept);
			...
			// timestamp="1247641769801" type="sim-end">
			...
			percept = new Percept("result", new Identifier(attributes
					.getNamedItem("result").getNodeValue()));
			ret.add(percept);

			percept = new Percept("averageScore", new Numeral(new Integer(
					attributes.getNamedItem("averageScore").getNodeValue())));
			ret.add(percept);
			...
			// <perception deadline="1246987323151" id="11" posx="11" posy="17"
			// score="0" step="10">
			// <cell x="-3" y="-3">
			// <obstacle/>
			// </cell>
			...
			int step = new Integer(attributes.getNamedItem("step")
					.getNodeValue());
				...
				if (cellNodes.item(a).getNodeName().equals("cell")) {
					...
					for (int b = 0; b < cellNode.getChildNodes().getLength(); b++) {
						...
						if (str.equals("empty"))
							...
							String agentType = nab.getAttributes()
							...
							String corralType = nab.getAttributes()
									.getNamedItem("type").getNodeValue();
							...
							String fenceOpen = nab.getAttributes()
									.getNamedItem("open").getNodeValue();
					...
					Percept cell = new Percept("cell", new Numeral(new Integer(
							attributesCell.getNamedItem("x").getNodeValue())),
							new Numeral(new Integer(attributesCell
									.getNamedItem("y").getNodeValue())),
							new Identifier(item), new Numeral(step));
			...
			// "id",
			// new Numeral( new
			// Integer(attributes.getNamedItem("id").getNodeValue()) )
			// );
			// ret.add(percept);

			// pos
			percept = new Percept("pos", new Numeral(new Integer(attributes
					.getNamedItem("posx").getNodeValue())),
					new Numeral(new Integer(attributes.getNamedItem("posy")
							.getNodeValue())), new Numeral(step));
			ret.add(percept);

			// currentScore
			percept = new Percept("cowsInCorral", new Numeral(new Integer(
					attributes.getNamedItem("cowsInCorral").getNodeValue())),
					new Numeral(step));
			ret.add(percept);

			// currentStep
			percept = new Percept("currentStep", new Numeral(new Integer(
					attributes.getNamedItem("step").getNodeValue())));
			ret.add(percept);
